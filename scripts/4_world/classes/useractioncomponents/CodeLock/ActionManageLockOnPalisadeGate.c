#ifdef CodeLock
class ActionManageLockOnPalisadeGate : ActionInteractBase
{
            ...
            if (codelock) {
                if (codelock.GetLockState()) {
                    if (codelock.IsOwner(identity.GetId())) {
                        GetCLUIManager().ShowMenu(CLMENU.ENTER_CODE_MENU, gate);
                    } else if (codelock.HasNoOwner()) {
                        GetCLUIManager().ShowMenu(CLMENU.CLAIM_CODE_MENU, gate);
                    }
                }
            }
        }
